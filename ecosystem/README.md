...
| **35** | [chain-solvency-invariant](35-chain-solvency-invariant.md) | **What the estate must be able to prove before it credits a coin it holds.** The invariant, the gaps against it (G1–G6), and what a newly added chain owes: a ledger `chain_assets` row, an *observable* balance (derived unspent-output totals for UTXO families, `eth_getBalance` for EVM), two coverage proofs, a pinned **and booked** treasury, and the `byLabelPrefix` breakdown crossing to the ledger. **Read this before adding any chain.** Its trap is that naming an asset for reconciliation before it can be observed freezes that asset permanently. |
| **36** | [multi-chain-and-mining-pool](36-multi-chain-and-mining-pool.md) | **Roadmap track 4: Bitcoin, Dogecoin and Ethereum Classic beside Litecoin — and a mining pool that is actually a mining pool.** §2 records that **Litecoin is not finished**: no withdrawal can be broadcast (no `LTC_RPC_URL`, so `NoEndpointError`) and deposits credit with no solvency check (`LEDGER_RECONCILE_ASSETS` does not name it). §3 costs each new chain — BTC is configuration only, DOGE is a UTXO chain that is *not* Litecoin (no bech32, coin type 3), ETC reuses the generic EVM path and needs a much deeper confirmation depth than Ethereum because of its 51% history. §5 states the arithmetic that makes browser mining of those four chains impossible rather than merely hard, and proposes the honest alternative: a real Stratum pool for real hardware, crediting ledger balances. **Partly a ledger now:** the pool shipped in release 2026.08.21 as `micro-pool` and `micro-pool-web` — Stratum v1 on 3333 (BTC) and 3334 (LTC), Dogecoin merge-mined as an *auxiliary* of Litecoin under AuxPoW rather than as a chain of its own, ETC and EMBER refused by name with the reason attached, MWEB mandatory on every Litecoin template. §5.3.1 records what shipped and the four places it diverges from the plan. §2's two blockers are also closed (§2.1a): settlement sends the `Authorization` header `URL.origin` was discarding, so LTC and BTC withdrawals build, and `LEDGER_RECONCILE_ASSETS` names `LTC`. The rest of the document is still a plan. |

| **37** | [legal-review-disposition](37-legal-review-disposition.md) | **The legal surface, and the fact that no lawyer has ever seen it.** The disposition of micro-org#33: 43 published sections across `/terms`, `/privacy` and `/risk`, of which **16 are `counsel` with deliberately empty bodies**; what a reader is told today; and the three things that are worse than the missing sections — a `stated` promise that *"the right itself is not ours to withhold"* against live freeze machinery, a cookies sentence that was true when written and is not now, and the finding that **no user has ever agreed to any of it** (`terms` occurs zero times in `micro-identity`). §5 lists the five controls that must not be softened to make the pages look finished; §7 maps #33, #161, #165, #316 and #313 onto each other and gives the order that wastes least of a lawyer's time. **A record, not advice — nothing in it closes anything.** |
| **38** | [combined-view](38-combined-view.md) | **One frontend estate, two networks — the topology that replaced doc 26's duplicated model** (release 2026.08.37). One set of surfaces on the mainnet hostnames with an in-app Mainnet/Testnet switcher; one shared identity separated at the token layer by the service-only `net` claim; the testnet frontends retired behind 302s while the testnet money tier and its `/v1` APIs survive as the data plane. Records the owner's decision, the security model, why the flip was one deploy, what deliberately survives (the amber band, the no-persistence invariant, the write asymmetry), and the rollback path (`runbook-combined-view-rollback.md`). |
| **39** | [forge-exchange](39-forge-exchange.md) | **An AMM on Hearth, and what has to be true before it opens.** Design authority for Forge Exchange. §1 corrects an understatement the marketing page shipped with — the contracts are *written* (`hearth/contracts/src/`, a Uniswap V2 port) and `hearth/node/test/dex.js` passes 167/167, which is the phase-7 gate of the EVM specification and the strongest claim anyone has made about this interpreter. §1.1 states what a green run deliberately does *not* prove. The rest is the gap between "the contracts run" and "a stranger can swap", and **none of it is contract work**. §4 is long because **35** governs it — an AMM pair is a second place a coin's balance lives, and the solvency invariant does not exempt it. Liquidity comes from the project's own mining, by the owner's decision, not from a sale. **Phases A–D are met as of 2026-08-15**: the multisig, the read surfaces, the full contract set on testnet 7412, and one funded EMBER/FTEST pool that a wallet holding a key we do not has traded both ways and provided liquidity to, from the browser extension, in blocks 17022–17034 (§6 of the plan; `micro-deploy/docs/hearth-exchange.md` is the record). **Mainnet is untouched, no frontend serves it, and `exchange.<apex>` still resolves nowhere on purpose.** |
| **40** | [forge-journal](40-forge-journal.md) | **A publication the estate can be found through.** Design authority for Forge Journal, the public archive at `journal.<apex>`. §1 states the gap as a fact rather than a wish: every public surface here is a *tool*, reached by somebody who already knows CloudsForge exists, so the estate is absent from every question a stranger actually types. §4 is the architecture and the only part that is not a preference — a client-rendered archive hands every link-preview fetcher one identical document for every address, so `scripts/prerender.ts` writes a real file per page and **the file set IS the route set** (`/a/an-article-never-written` answers a real 404, which an SPA fallback cannot). §4.1 records the three load-bearing nginx lines behind `__CF_ORIGIN__`, including the one that is an *absence*. §2 and §5 are the refusals: no CMS, no comments, no scheduling, no announcements, and **no page that names a return, a forecast or a recommendation** — nine patterns enforced in `test/content.test.ts` and again in CI, because this is published by a company that operates custody. Five articles, five topics, and a claim register cross-checked against micro-site's. **Built, green, and deployed nowhere: §6 lists the estate registration still outstanding, including two DNS records only the owner can create.** |
...
**35 and 36 were written after them** (2026-08-07 and 2026-08-08) and are narrower: each is about
one invariant and the work it implies, rather than a sweep of the estate.
