...
| **39** | [forge-exchange](39-forge-exchange.md) | **An AMM on Hearth, and what has to be true before it opens.** Design authority for Forge Exchange. §1 corrects an understatement the marketing page shipped with — the contracts are *written* (`hearth/contracts/src/`, a Uniswap V2 port) and `hearth/node/test/dex.js` passes 167/167, which is the phase-7 gate of the EVM specification and the strongest claim anyone has made about this interpreter. §1.1 states what a green run deliberately does *not* prove. The rest is the gap between "the contracts run" and "a stranger can swap", and **none of it is contract work**: nothing is deployed on either chain, there is no pool, no frontend, no repository for one, and nothing to trade EMBER *against*. §4 is long because **35** governs it — an AMM pair is a second place a coin's balance lives, and the solvency invariant does not exempt it. Liquidity comes from the project's own mining, by the owner's decision, not from a sale. **A plan. Nothing is deployed, and the reserved hostname resolves nowhere on purpose.** |
